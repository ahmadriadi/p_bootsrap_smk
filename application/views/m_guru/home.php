...
<script>
    ...
    var ParamFunc = {
        ...
        deletedata: function (id) {
            $('#confirmMguru').modal('show');
            $("#confirmMguru  input[name=guru_id]").val(id);
        }
    }
    ...
    $(document).ready(function () {
        ...
        $('#gridData').DataTable({
            ...
            "columns": [
                ...
                {"data": "agama"},
                {
                    "data": "guru_id", "width": "100px", "sClass": "left",
                    "bSortable": false,
                    "mRender": function (data, type, row) {
                        var btn = "";
                        btn = btn + "<div class='btn-group'>";
                        btn = btn + " <button onClick='ParamFunc.editdata(" + row.guru_id + ")' class='btn btn-xs btn-info' type='button'>Edit</button>";
                        btn = btn + " <button onClick='ParamFunc.deletedata(" + row.guru_id + ")' class='btn btn-xs btn-info' type='button'>Delete</button>";
                        btn = btn + "</div>";
                        return btn;
                    }
                }
        ...
        //start delete data
        $("#confirmMguru button[action=delete]").click(function () {
            var url;
            ...
            $.ajax({
                ...
                cache: false,
                data: {
                    guru_id: $("#confirmMguru input[name=guru_id]").val()
                },
                success: function (data) {
                    $('#confirmMguru').modal('hide');
