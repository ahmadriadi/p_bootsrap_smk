...
<script type="text/javascript">
 ...
 $('#gridMatPel').DataTable({
        ...
        "columns": [
            {"data": "nama"},
            {
                "data": "agama_id", "width": "100px", "sClass": "left",
                "bSortable": false,
                "mRender": function (data, type, row) {
                    var btn = "";
                    btn = btn + "<div class='btn-group'>";
                    btn = btn + " <button onClick='ParamFunc.editdata(" + row.agama_id + ")' class='btn btn-xs btn-info' type='button'>Edit</button>";
                    btn = btn + " <button onClick='ParamFunc.deletedata(" + row.agama_id + ")' class='btn btn-xs btn-info' type='button'>Delete</button>";
                    btn = btn + "</div>";
                    return btn;
                }
            }
    ...
    var ParamFunc = {
        ...
        deletedata: function (id) {
            $('#confirmAgama').modal('show');
            $("#confirmAgama  input[name=agama_id]").val(id);
        }
    }
    
    //start delete data
        $("#confirmAgama button[action=delete]").click(function () {
            var url;
            ...
            $.ajax({
                ...
                cache: false,
                data: {
                    agama_id: $("#confirmAgama input[name=agama_id]").val()
                },
                success: function (data) {
                    $('#confirmAgama').modal('hide');
